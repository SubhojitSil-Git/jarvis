...
const Engine3D = {
    ...
    render() {
        ...
        for (let i = 0; i < State.particles.count; i++) {
            ...
            pAttr[idx] += (tx - pAttr[idx]) * 0.12;
            pAttr[idx+1] += (ty - pAttr[idx+1]) * 0.12;
            pAttr[idx+2] += (tz - pAttr[idx+2]) * 0.12;

            if (State.rainbowMode) {
                const pCol = new THREE.Color().setHSL((i / State.particles.count) + (Date.now() * 0.0004), 0.7, 0.5);
        ...
        if (State.gesture !== 'PALM_OPEN') this.points.rotation.y += 0.004 + (bass * 0.04);
