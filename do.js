...
const SoundEngine = {
    playStartup() {
        ...
        const msg = new SpeechSynthesisUtterance("Neural Link Established. Glow deactivated.");
...
const Engine3D = {
    ...
    createIronManMatrix() {
        ...
        // Slightly larger size for better visibility without glow
        const mat = new THREE.PointsMaterial({
            size: 1.8, 
            vertexColors: true, 
            transparent: true, 
            opacity: 0.9,
            blending: THREE.NormalBlending, // Changed from Additive to prevent over-brightness
            depthWrite: false
        });
    ...
    setupPostProcessing() {
        ...
        // Strength set to 0 to completely disable glow
        const bloom = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0, 0, 1.0);
